...
def clacSimilaryScore(model, v, len):
    li = model.wv.similar_by_vector(np.array(model[v]), topn=len) # topn=None为计算所有
    return li
...
# 遍历所有，返回TOPＫ
def getSimilaryByMostKSimilary(model, g=nx.karate_club_graph(), k=6):
    result_dict = dict()
    for i in g.nodes:
        result_dict[str(i)] = 0
    ...
    for rand_ in range(3):
        ...
        theta = clacSimilaryScore(model, str(random_node), len(g))[k][1]
        for i in g.nodes:
